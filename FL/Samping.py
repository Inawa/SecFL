...
def split_ATT(dataset, num_users):
    num_items = int(len(dataset)/num_users)
    dict_users, all_idxs = {}, [i for i in range(len(dataset))]
    start = 0
    for i in range(num_users):
        dict_users[i] = all_idxs[start:start+num_items]
        start+=num_items
    return dict_users
...
def mnist_noniid2(dataset, num_users=10):
    ...
    for i in range(10):
        pos = idxs_labels[1].tolist().index(i)
        idxs_new = np.concatenate((idxs_new, idxs[pos:pos+5000]), axis=0).astype('int64')
    ...
    for i in range(num_users):
        ...
        for rand in rand_set:
            dict_users[i] = np.concatenate((dict_users[i], idxs_new[rand*num_imgs:(rand+1)*num_imgs]), axis=0)
